...
namespace ScentAir.Payment.Impl
{
    public class SFAccountSettingsManager : ISFAccountSettingsManager
    {
        ...
        public async Task<ITaskResult> SaveSFAccountSettingsAsync(SFAccountSettings sfAccountSettings, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            dbSFAccountSettings = await portalContext
                .SFAccountSettings
                .Where(x => x.Id == sfAccountSettings.Id)
                .Include(x => x.Id)
                .Include(x => x.BillingLine1)
                .Include(x => x.BillingLine2)
                .Include(x => x.BillingLine3)
                .Include(x => x.BillingMunicipality)
                .Include(x => x.BillingStateOrProvince)
                .Include(x => x.BillingPostalCode)
                .Include(x => x.BillingCountry)
                .Include(x => x.ShippingLine1)
                .Include(x => x.ShippingLine2)
                .Include(x => x.ShippingLine3)
                .Include(x => x.ShippingMunicipality)
                .Include(x => x.ShippingStateOrProvince)
                .Include(x => x.ShippingPostalCode)
                .Include(x => x.ShippingCountry)
            ...
            else
            {
                ...
                dbSFAccountSettings.CopyNotNull(x => x.BillingMunicipality, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.BillingStateOrProvince, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.BillingPostalCode, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.BillingCountry, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.ShippingLine1, sfAccountSettings);
                ...
                dbSFAccountSettings.CopyNotNull(x => x.ShippingMunicipality, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.ShippingStateOrProvince, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.ShippingPostalCode, sfAccountSettings);
                dbSFAccountSettings.CopyNotNull(x => x.ShippingCountry, sfAccountSettings);

                //dbSFAccountSettings.CopyNotEmpty(x => x.CustomerPurchaseOrderNumber, sfAccountSettings);
